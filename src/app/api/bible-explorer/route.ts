  ...
  try {
    ...
    if (!problem || typeof problem !== "string") {
      return NextResponse.json(
        { error: "Please share what you want to explore." },
        { status: 400 }
      );
    }
    ...
    if (problem.length > 250) {
      return NextResponse.json(
        { error: "Please keep your entry to 250 characters or less." },
        { status: 400 }
      );
    }
      ...
      input: [
        {
          role: "system",
          content: `
You are a careful Scripture discovery facilitator for Cross Heart Pray Holy Bible Explorer.

Mission:
- Cross Heart Pray Holy Bible Explorer helps users discover Scripture, read Scripture in context, pray through Scripture, and take a faithful next step toward Jesus.
...
Your role:
- Read the user's actual question, topic, season, struggle, decision, word, verse, or place they want to begin.
- Identify what is reasonably present: Bible questions, themes, seasons, decisions, relationships, responsibilities, opportunities, strengths, weaknesses, fears, hopes, doubts, gratitude, habits, character, spiritual questions, and biblical themes.
...
- The explorer reflects what was written; it does not pretend to know the heart. God knows the heart.
...
Do not include commentary.
    ...
    try {
      ...
      console.error("BIBLE EXPLORER PARSE ERROR:", parseError);
      console.error("BIBLE EXPLORER RAW OUTPUT:", response.output_text);
      throw new Error("Structured Bible Explorer response could not be parsed.");
    }
...
`;
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Bible Explorer is connected, but the AI account needs API billing or quota added before Scripture exploration can be generated.",
        },
        { status: 429 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Unable to explore Scripture." },
      { status: 500 }
    );
  }
}
